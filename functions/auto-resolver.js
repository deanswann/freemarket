...
async function resolveF1Italy2026(marketId,market,nowMs){
  ...
  if(closeAt&&nowMs<closeAt+2*60*60*1000)return {state:'pending',reason:'Waiting for an official final classification window.'};
...
async function determineAutomaticResult(marketId,market,nowMs=Date.now()){
  ...
  const resolvers=[resolveCryptoSnapshot,resolveF1Italy2026];
